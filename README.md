# 법률-시행령 형식 기준 804건 최종 F분류 탐색기

**GitHub Pages:** <https://hosungseo.github.io/local-delegation-field-admin/>

사무 성격 판단은 제외하고, 전체 804개 사무를 대상으로 F4 제거와 F5 회복매칭을 반영한 최종 F분류 정적 탐색기입니다.
...
- 통합 매트릭스: 법률상 지자체 유형 × 시행령 현재 상태 × 사무 성격
...
- 법률 조문 데이터: 263개 법률 수록
- 시행령 조문 데이터: 225개 시행령 수록
- 법률 조문 키 매칭: 774건
- 시행령 조문 키 매칭: 527건
- 원문 출처: 법제처 국가법령정보 API(law.go.kr) 현행 법령 조문
- 일부 오기·폐지법령·시행규칙명 등은 법제처 검색 미매칭으로 근거 요약만 표시
...
이 레포에는 법제처 조문 데이터를 포함한 정적 JSON `app/data/delegation.json`을 포함합니다.
